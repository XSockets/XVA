...
if((Test-Path $config) -eq $true){   
    ...
    if(($xml.configuration['appSettings'].add | Where-Object {$_.key -eq 'xsockets.license.path'}) -eq $null){
        $pluginCatalog = $xml.CreateElement('add')
        $pluginCatalog.setAttribute('key','xsockets.license.path')
...
if($webproject -eq $true){
    ...
    #Write-Host "New to XSockets.NET? Visit http://xsockets.net/docs/5/xsocketsnet-5-documentation " -ForegroundColor Blue    
    #Write-Host "########################################################################################################################" -ForegroundColor Green
}
